...
% scannerTranslation = [X Y Z];
% scannerTransform = [eye(3), scannerTranslation'; 0,0,0,1];
scanner = PlaceObject('barcodescanner3.ply', [X, Y, Z]);
% scannerVerts = [get(scanner, 'Vertices'), ones(size(get(scanner, 'Vertices'), 1), 1)];
% scannernewVerts = (scannerVerts(:, 1:3) * scannerTransform(1:3, 1:3)') + scannerTransform(1:3, 4)';
% set(scanner, 'Vertices', scannerVerts);
...
for i = 1:size(qtraj1, 1)
    ...
    % Delete the previous scanner object
